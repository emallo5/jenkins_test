...
public abstract class StringUtils {
	...
	public static Boolean equalsIgnoreCase(String text1, String text2) {
		return defaultString(text1).equalsIgnoreCase(defaultString(text2));
	}
	...
	public static Collection<String> removeBlankStrings(Collection<String> strings) {
		Collection<String> objectsNotBlank = Lists.newArrayList();
		for (String string : strings) {
			if (isNotBlank(string)) {
				objectsNotBlank.add(string);
			}
		}
		return objectsNotBlank;
	}
	
	public static String joinIgnoreBlanks(String separator, String... strings) {
		return joinIgnoreBlanks(Arrays.asList(strings), separator);
	}
	
	public static String joinIgnoreBlanks(Collection<String> strings, String separator) {
		return join(removeBlankStrings(strings), separator);
	}
	...
	/**
	 * 
	 * @param string is the string that is going to be modify
	 * @param maskChar is the character that will be use to transform original string
	 * @param maskLenght should be a number between 0 and string.lenght() - 1
	 * @return string transform with a substring form with the charToSet
	 */
	public static String maskFirstCharacters(String string, String maskChar, int maskLenght) {
		
		String maskedString = "";
		int minMaskLenght = Math.min(maskLenght, string.length());
		
		for (int i = 0; i < minMaskLenght; i++) {
			maskedString += maskChar;
		}
		maskedString += string.substring(minMaskLenght);
		
		return maskedString;
	}
	
	/**
	 * @param charSecuence the CharSequence to check, may be null
	 * @return {@code true} if only contains digits, and is non-null
	 */
	public static boolean isNumeric(CharSequence charSecuence) {
		if ((charSecuence == null) || (charSecuence.length() == 0)) {
			return false;
		}
		int size = charSecuence.length();
		for (int i = 0; i < size; i++) {
			if (Character.isDigit(charSecuence.charAt(i)) == false) {
				return false;
			}
		}
		return true;
	...
	/**
	 * @param text a String, the text to trim.
	 * @return a string with whitespace characters removed from the end, including non-breaking spaces (&nbsp; or
	 *         &#160;) and line separators.
	 */
	public static String removeTrailingWhitespaces(String text) {
		String result = text;
		if ((text != null) && (text.length() > 0)) {
			int lastIndex = text.length() - 1;
			int index = lastIndex;
			while ((index >= 0) && isAnySpaceChar(text.charAt(index))) {
				index--;
			}
			if (index < lastIndex) {
